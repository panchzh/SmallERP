...
namespace SMALLERP.ComClass
{
    ...
    public class CommonUse
    {
        ...
        public void BindComboBox(Object obj, string strValueColumn, string strTextColumn, string strSql, string strTable)
            //Component —替换—> Object
        {
            try
            {
        ...
        public string BuildBillCode(string strTable, string strBillCodeColumn, string strBillDateColumn,
                                    DateTime dtBillDate)
        {
            string strSql;
            string strBillDate;
            ...
            try
            {
                ...
                strSql = "SELECT  SUBSTRING(MAX(" + strBillCodeColumn + "),10,4) FROM " + strTable + " WHERE " +
                         strBillDateColumn + " = '" + dtBillDate.ToString("yyyy-MM-dd") + "'";
        ...
        public decimal GetAvePriceBySTGetMaterial(string strPRProduceCode, string strInvenCode)
        {
            ...
            strSql =
                "SELECT SUM(UnitPrice * Quantity) / SUM(Quantity) FROM STGetMaterial WHERE BillType = '1' AND IsFlag = '1'  AND PRProduceCode = '" +
        ...
        public bool IsExistConstraint(string strPrimaryTable, string strPrimaryValue)
        {
            ...
            try
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    strSql = "Select " + strForeignColumn + " From " + strForeignTable + " Where " + strForeignColumn +
                             " = '" + strPrimaryValue + "'";
        ...
        public void CortrolButtonEnabled(IComponent iComp, Form form)
        {
            ...
            else
            {
                string strSql = "Select IsRight From SYAssignRight Where OperatorCode = '" + PropertyClass.OperatorCode +
                                "' ";
                strSql += "and ModuleTag = '" + form.Tag + "'and RightTag = '" + strRightTag + "'";
